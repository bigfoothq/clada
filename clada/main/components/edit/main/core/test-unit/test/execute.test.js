...
describe('executeEdit', () => {
  ...
  it('performs basic exact replacement', () => {
    ...
    const result = executeEdit({ mode: 'exact', path: 'test.txt', search: 'hello', replace: 'hi', count: 1 }, context);
    assert.deepEqual(result, { ok: true, value: undefined });
    assert.equal(readFileSync(join(testDir, 'test.txt'), 'utf8'), 'hi world hello');
  });

  it('performs exact replacement with count', () => {
    writeFileSync(join(testDir, 'data.txt'), 'foo bar foo baz foo');
    const result = executeEdit({ mode: 'exact', path: 'data.txt', search: 'foo', replace: 'qux', count: 2 }, context);
    assert.deepEqual(result, { ok: true, value: undefined });
    assert.equal(readFileSync(join(testDir, 'data.txt'), 'utf8'), 'qux bar qux baz foo');
  });
